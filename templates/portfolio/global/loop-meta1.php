...
if (filter_var($show_metadata, FILTER_VALIDATE_BOOLEAN) && !empty($metadata)) {
    foreach ($metadata as $meta) {
        ...
        switch ($item_type) {
            case 'category':
                $meta_value = get_the_term_list(get_the_ID(), 'nova_portfolio_category', '', '<span class="cspr">, </span>');
    ...
    if (!empty($output)) {
        echo sprintf('<div class="kitify-posts__meta kitify-posts__meta1">%s</div>', $output);
    }
}