...
class Group_Control_Related extends Group_Control_Query {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type']['options']['related'] = __( 'Related', 'kitify' );
		...
		$related_fallback = [
			'label' => __( 'Fallback', 'kitify' ),
			'type' => Controls_Manager::SELECT,
			'options' => [
				'fallback_none' => __( 'None', 'kitify' ),
				'fallback_by_id' => __( 'Manual Selection', 'kitify' ),
				'fallback_recent' => __( 'Recent Posts', 'kitify' ),
			],
			'default' => 'fallback_none',
			'description' => __( 'Displayed if no relevant results are found. Manual selection display order is random', 'kitify' ),
			'condition' => [
				'post_type' => 'related',
			],
			'separator' => 'before',
	...
	protected function get_supported_taxonomies() {
		...
		$public_types = kitify_helper()->get_post_types();
