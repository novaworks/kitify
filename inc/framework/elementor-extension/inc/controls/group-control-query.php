<?php
namespace Kitify_Extension\Controls;
...
class Group_Control_Query extends Group_Control_Base {
	...
	protected function init_fields_by_name( $name ) {
		...
		$fields['post_type'] = [
			...
			'options' => [
				'by_id' => __( 'Manual Selection', 'kitify' ),
		...
		$fields['include'] = [
			'label' => __( 'Include By', 'kitify' ),
			'type' => Controls_Manager::SELECT2,
			'multiple' => true,
			'options' => [
				'terms' => __( 'Term', 'kitify' ),
				'authors' => __( 'Author', 'kitify' ),
			],
			'condition' => [
				'post_type!' => [
					'by_id',
		...
		$fields['avoid_duplicates'] = [
			'label' => __( 'Avoid Duplicates', 'kitify' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => '',
			'description' => __( 'Set to Yes to avoid duplicate posts from showing up. This only effects the frontend.', 'kitify' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
			'condition' => [
				'post_type!' => [
		...
		$fields['offset'] = [
			...
			'description' => __( 'Use this setting to skip over posts (e.g. \'2\' to skip over 2 posts).', 'kitify' ),
			'tabs_wrapper' => $tabs_wrapper,
			'inner_tab' => $exclude_wrapper,
		];

		$fields['select_date'] = [
			'label' => __( 'Date', 'kitify' ),
			'type' => Controls_Manager::SELECT,
			'post_type' => '',
			'options' => [
				'anytime' => __( 'All', 'kitify' ),
				'today' => __( 'Past Day', 'kitify' ),
				'week' => __( 'Past Week', 'kitify' ),
				'month'  => __( 'Past Month', 'kitify' ),
				'quarter' => __( 'Past Quarter', 'kitify' ),
				'year' => __( 'Past Year', 'kitify' ),
				'exact' => __( 'Custom', 'kitify' ),
			],
			'default' => 'anytime',
			'label_block' => false,
			'multiple' => false,
		...
		$fields['date_before'] = [
			...
			'description' => __( 'Setting a ‘Before’ date will show all the posts published until the chosen date (inclusive).', 'kitify' ),
		];

		$fields['date_after'] = [
			'label' => __( 'After', 'kitify' ),
			'type' => Controls_Manager::DATE_TIME,
			'post_type' => '',
			'label_block' => false,
			'multiple' => false,
			...
			'description' => __( 'Setting an ‘After’ date will show all the posts published since the chosen date (inclusive).', 'kitify' ),
		];
		...
		$fields['orderby'] = [
			...
			'default' => 'post_date',
			'options' => [
				'post_date' => __( 'Date', 'kitify' ),
				'post_title' => __( 'Title', 'kitify' ),
		...
		$fields['posts_per_page'] = [
			'label' => __( 'Posts Per Page', 'kitify' ),
			'type' => Controls_Manager::NUMBER,
			'default' => 3,
			'condition' => [
				'post_type!' => 'current_query',
		...
		$fields['ignore_sticky_posts'] = [
			'label' => __( 'Ignore Sticky Posts', 'kitify' ),
			'type' => Controls_Manager::SWITCHER,
			'default' => 'yes',
			'condition' => [
				'post_type' => 'post',
			],
			'description' => __( 'Sticky-posts ordering is visible on frontend only', 'kitify' ),
		];

		$fields['query_id'] = [
			'label' => __( 'Query ID', 'kitify' ),
			'type' => Controls_Manager::TEXT,
			'default' => '',
			'description' => __( 'Give your Query a custom unique id to allow server side filtering', 'kitify' ),
			'separator' => 'before',
		];
	...
	protected function prepare_fields( $fields ) {
		...
		$post_types = kitify_helper()->get_post_types( $post_type_args );
