...
if ( ! class_exists( 'CX_Customizer' ) ) {
	...
	class CX_Customizer {
		...
		public function customize_preview_init(){
			$theme_version = defined('WP_DEBUG') && WP_DEBUG ? time() : kitify()->get_version();
			$dependency = array(
				'jquery',
				'customize-preview',
			);
			wp_enqueue_script( 'kitify-customize-preview', kitify()->plugin_url('assets/js/nova-customizer-preview.js') , $dependency, $theme_version, true );
			wp_localize_script('kitify-customize-preview', 'kitifyCustomizeConfigs', $this->get_controls_with_css());
		}
		...
		public function auto_enqueue_frontend_css(){
			...
			foreach ( (array) $this->options as $id => $option ) {
				...
				if ( 'control' === $option['type'] && !empty($option['css']) ) {
					...
					if( $option['field'] === 'kitify_responsive' && $option['responsive'] ) {
			...
			if(!empty($css_render)){
				echo sprintf('<style id="kitifycustomizer-css-style">%1$s</style>', $css_render);
			}
		}
		...
		public function add_control( $id, $args ) {
            ...
            if ( isset( $args['responsive'] ) && $args['responsive'] ) {
                ...
                $settings_tablet_ext            = $args_settings;
                $settings_tablet_ext['default'] = isset( $settings_tablet_ext['default_tablet_extra'] ) ? $settings_tablet_ext['default_tablet_extra'] : $settings_tablet_ext['default'];
                $this->customize->add_setting( $id . '_tablet_extra', $settings_tablet_ext );
			...
			switch ( $field_type ) {
				...
				default:
						...
						$control_args = apply_filters(
							'kitify/theme/customizer/control_args/{$field_type}',
							$control_args,
							$id,
							$this
						);
			 ...
			 * @param object $this         Cherry_Customizer instance.
			 */
			$control_args = apply_filters( 'kitify/theme/customizer/control_args', $control_args, $id, $this );
			 ...
			 * @param object $this         Cherry_Customizer instance.
			 */
			$control_class = apply_filters( 'kitify/theme/customizer/control_class', $control_class, $id, $this );
		...
		public function get_control_section( $args ) {
			...
			$default_sections = apply_filters( 'kitify/theme/customizer/core_sections', array(
				'title_tagline',
				'colors',
				'header_image',
				'background_image',
		...
		public function get_image_types() {
			 ...
			 */
			return apply_filters( 'kitify/theme/customizer/image_types', array(
		...
		public function prepare_fonts() {
			 ...
			 */
			$this->fonts = apply_filters( 'kitify/theme/customizer/fonts_list', $this->fonts, $this );
		...
		public function get_fonts_data() {
			 ...
			 * @param object $this CX_Customizer instance.
			 */
			return apply_filters( 'kitify/theme/customizer/fonts_data', array(
        ...
        public function custom_devices( $devices ) {
            $laptop           = array(
                'laptop' => array(
                    'label' => __( 'Enter laptop preview mode', 'kitify' ),
                ),
            );
            $mobile_landscape = array(
                'mobile_extra' => array(
                    'label' => __( 'Enter mobile landscape preview mode', 'kitify' ),
                ),
            );
