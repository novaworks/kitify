 ...
 * Walker class
 */
class Kitify_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_lvl( &$output, $depth = 0, $args = array() ) {
		...
		// Default class.
		$classes     = array( 'kitify-nav__sub' );
		$classes[]   = 'kitify-nav-depth-' . $depth;
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$classes[] = 'kitify-nav__item-' . $item->ID;
		$classes[] = 'kitify-nav__item';
		if ( 0 < $depth ) {
			$classes[] = 'kitify-nav-item-sub';
		}
		...
		if ( ! empty( $item->description ) && $show_desc ) {
			$desc = sprintf(
				'<span class="kitify-nav-item-desc %2$s">%1$s</span>',
				$item->description,
				( 0 === $depth ) ? 'top-level-desc'  : 'sub-level-desc'
			);
		}

		$title = sprintf( '<span class="kitify-nav-link-text">%s%s</span>', $title, $desc );
		...
		if ( in_array( 'menu-item-has-children', $item->classes ) ) {
			$arrow_icon = isset( $args->widget_settings['dropdown_icon'] ) ? $args->widget_settings['dropdown_icon'] : 'novaicon-down-arrow';
	...
	public function get_dropdown_arrow_html( $icon = '' ) {
		...
		$format = apply_filters(
			'kitify/nav-menu/dropdown-arrow-format',
