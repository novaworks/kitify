...
use KitifyThemeBuilder\Modules\ThemeBuilder\Classes\Conditions_Manager;
use KitifyThemeBuilder\Modules\ThemeBuilder\Module as ThemeBuilderModule;
use KitifyThemeBuilder\Modules\ThemeBuilder\Classes\Templates_Types_Manager;
use KitifyThemeBuilder\Modules\AdminApp\Modules\SiteEditor\Render_Mode_Template_Preview;
use KitifyThemeBuilder\Modules\AdminApp\Modules\SiteEditor\Data\Responses\Lock_Error_Response;
...
class Templates extends Base_Endpoint {
	...
	public function __construct( $controller ) {
		...
		$this->templates_manager = kitify()->elementor()->templates_manager;
	}
	...
	private function normalize_template_json_item( $template ) {
		...
		$conditions_manager = kitify()->modules_manager->get_modules( 'theme-builder' )->get_conditions_manager();

		/** @var Theme_Document $document */
		$document = kitify()->elementor()->documents->get( $template['template_id'] );
		...
		if ( ! $has_instances ) {
			$template['instances'] = [ 'no_instances' => esc_html__( 'No instances', 'kitify' ) ];
		} else {
		...
		return apply_filters( 'kitify/site-editor/data/template', $data );
	}
