<?php
namespace KitifyThemeBuilder\Modules\ThemeBuilder;
...
use KitifyThemeBuilder\Modules\ThemeBuilder\Classes;
use KitifyThemeBuilder\Modules\ThemeBuilder\Documents\Single;
use KitifyThemeBuilder\Modules\ThemeBuilder\Documents\Theme_Document;
...
class Module extends \Elementor\Core\Base\Module {
	...
	public static function is_preview() {
	    return kitify()->elementor()->preview->is_preview_mode() || is_preview();
	}

	public static function get_public_post_types( $args = [] ) {

	    $post_types = kitify_helper()->get_post_types();
	...
	public function get_document( $post_id ) {
		...
		try {
			$document = kitify()->elementor()->documents->get( $post_id );
		} catch ( \Exception $e ) {
			// Do nothing.
			unset( $e );
		}
	...
	public function localize_settings( $settings ) {
		$settings = array_replace_recursive( $settings, [
			'i18n' => [
				'publish_settings' => __( 'Publish Settings', 'kitify' ),
				'conditions' => __( 'Conditions', 'kitify' ),
				'display_conditions' => __( 'Display Conditions', 'kitify' ),
				'choose' => __( 'Choose', 'kitify' ),
				'add_condition' => __( 'Add Condition', 'kitify' ),
				'conditions_title' => __( 'Where Do You Want to Display Your %s?', 'kitify' ),
				'conditions_description' => __( 'Set the conditions that determine where your %s is used throughout your site.', 'kitify' ) . '<br>' . __( 'For example, choose \'Entire Site\' to display the template across your site.', 'kitify' ),
				'conditions_publish_screen_description' => __( 'Apply current template to these pages.', 'kitify' ),
				'save_and_close' => __( 'Save & Close', 'kitify' ),
				'open_site_editor' => __( 'Open Site Editor', 'kitify' ),
	...
	public function create_new_dialog_types( $types ) {
		...
		foreach ( $types as $type => $label ) {
			$document_type = kitify()->elementor()->documents->get_document_type( $type );
	...
	public function print_post_type_field() {
		...
		<div id="elementor-new-template__form__post-type__wrapper" class="elementor-form-field">
			<label for="elementor-new-template__form__post-type" class="elementor-form-field__label">
				<?php echo __( 'Select Post Type', 'kitify' ); ?>
			</label>
			<div class="elementor-form-field__select__wrapper">
				<select id="elementor-new-template__form__post-type" class="elementor-form-field__select" name="<?php echo Single::REMOTE_CATEGORY_META_KEY; ?>">
					...
					foreach ( $post_types as $post_type => $label ) {
						$doc_type = kitify()->elementor()->documents->get_document_type( $post_type );
					...
					// 404.
					echo sprintf( '<option value="%1$s">%2$s</option>', 'not_found404', __( '404 Page', 'kitify' ) );
	...
	public function on_elementor_editor_init() {
		kitify()->elementor()->common->add_template( __DIR__ . '/views/panel-template.php' );
	}

	public function add_finder_items( array $categories ) {
		$categories['create']['items']['theme-template'] = [
			'title' => __( 'Add New Theme Template', 'kitify' ),
			'icon' => 'plus-circle-o',
	...
	public function admin_menu() {
		add_submenu_page( 'kitify-dashboard-settings-page', '', __( 'Theme Builder', 'kitify' ), 'publish_posts', $this->get_admin_templates_url( true ) );
	}

	public function print_new_theme_builder_promotion( $views ) {
		/** @var Source_Local $source */
		$source = kitify()->elementor()->templates_manager->get_source( 'local' );
		...
		if ( self::ADMIN_LIBRARY_TAB_GROUP === $current_tab_group ) {
			 ...
			 */
			$admin_notices = kitify()->elementor()->admin->get_component( 'admin-notices' );

			$admin_notices->print_admin_notice( [
				'title' => __( 'Meet the New Theme Builder: More Intuitive and Visual Than Ever', 'kitify' ),
				'description' => __( 'With the new Theme Builder you can visually manage every part of your site intuitively, making the task of designing a complete website that much easier', 'kitify' ),
				'button' => [
					'text' => __( 'Try it Now', 'kitify' ),
					'class' => 'elementor-button elementor-button-success',
					'url' => kitify()->elementor()->app->get_settings( 'menu_url' ),
				],
			] );
		}
    ...
    public function enqueue_editor_scripts() {
        ...
        wp_enqueue_style(
            ...
            kitify()->plugin_url( 'assets/css/kitify-editor-conditions.css' ),
            [],
            kitify()->get_version()
        );
        ...
        $locale_settings = apply_filters('kitify_pro/admin/localize_settings', $locale_settings);
