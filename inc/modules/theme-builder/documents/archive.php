	...
	public static function get_preview_as_options() {
		...
		foreach ( $post_types as $post_type => $label ) {
			...
			if ( $post_type_object->has_archive ) {
				$post_type_archives[ 'post_type_archive/' . $post_type ] = sprintf( __( '%s Archive', 'kitify' ), $post_type_object->label );
			}
			...
			foreach ( $post_type_taxonomies as $slug => $object ) {
				$taxonomies[ 'taxonomy/' . $slug ] = sprintf( __( '%s Archive', 'kitify' ), $object->label );
			}
		}

		$options = [
			'archive/recent_posts' => __( 'Recent Posts', 'kitify' ),
			'archive/date' => __( 'Date Archive', 'kitify' ),
			'archive/author' => __( 'Author Archive', 'kitify' ),
