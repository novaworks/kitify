...
use KitifyThemeBuilder\Modules\ThemeBuilder\Conditions\Condition_Base;
...
class Conditions_Manager {
	...
	public function ajax_check_conditions_conflicts( $request ) {
		...
		return __( 'Elementor recognized that you have set this location for other templates: ', 'kitify' ) .
			' ' .
			implode( ', ', $conflicted );
	}

	public function get_conditions_conflicts( $post_id, $condition ) {
		/** @var \KitifyThemeBuilder\Modules\ThemeBuilder\Module $theme_builder_module */
		$theme_builder_module = Module::instance();
	...
	private function register_condition( $id, $args = [] ) {
		...
		$class_name = '\\KitifyThemeBuilder\\Modules\\ThemeBuilder\\Conditions\\' . $class_name;
	...
	public function get_documents_for_location( $location ) {
        ...
        $theme_templates_ids = apply_filters('kitify/theme_templates_ids', $theme_templates_ids, $location);
