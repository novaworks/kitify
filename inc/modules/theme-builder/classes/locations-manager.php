<?php
namespace KitifyThemeBuilder\Modules\ThemeBuilder\Classes;
...
class Locations_Manager {
	...
	public function enqueue_styles() {
		...
		if ( ! empty( $css_files ) ) {
			kitify()->elementor()->frontend->enqueue_styles();
	...
	public function template_include( $template ) {
		...
		if ( is_singular() ) {
			$document = kitify()->elementor()->documents->get_doc_for_frontend( get_the_ID() );
		 ...
		 */
		$page_templates_module = kitify()->elementor()->modules_manager->get_modules( 'page-templates' );
		...
		if ( $post_id ) {
			...
			if ( $document ) {
				...
				if ( $location_settings && ! $location_settings['edit_in_content'] ) {
					$content = '<div class="elementor-theme-builder-content-area">' . __( 'Content Area', 'kitify' ) . '</div>';
				}
			}
		}
	...
	public function register_core_location( $location, $args = [] ) {
		if ( ! isset( $this->core_locations[ $location ] ) ) {
			...
			wp_die( esc_html( sprintf( __( 'Location \'%s\' is not a core location.', 'kitify' ), $location ) ) );
		}
	...
	public function inspector_log( $args ) {
		$inspector_enabled = method_exists( kitify()->elementor()->inspector, 'is_enabled' ) && kitify()->elementor()->inspector->is_enabled();
		...
		if ( isset( $args['template'] ) ) {
			$title[] = kitify()->elementor()->inspector->parse_template_path( $args['template'] );
		}

		$title = implode( ' > ', $title );

		kitify()->elementor()->inspector->add_log( 'Theme', $title, $url );
	}
